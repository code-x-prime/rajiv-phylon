    ...
    <section className="py-14 md:py-20 border-t border-gray-100 bg-white">
      <div className="max-w-site mx-auto px-6 lg:px-10">
        ...
        <div className="flex items-end justify-between mb-8 md:mb-10">
          <div>
            <p className="type-overline text-[#F5B400] mb-3">
              Our Facility
            </p>
            <h2 className="font-display font-medium text-[clamp(1.25rem,2.5vw,2rem)] text-[#111111] tracking-[-0.02em]">
              Factory Gallery
            </h2>
          </div>
          <Link
            href="/gallery"
            className="hidden sm:flex items-center gap-1.5 text-sm font-display font-medium text-[#111111] border border-gray-200 rounded-xl px-4 py-2.5 hover:border-[#F5B400] hover:text-[#F5B400] transition-all duration-200 shrink-0"
          >
            View all
        ...
        <div className="md:hidden flex gap-3 overflow-x-auto pb-3 snap-x snap-mandatory scroll-smooth scrollbar-hide -mx-6 px-6">
          {list.map((item, i) => (
            <div
              key={item.id || i}
              ...
              <div className="relative aspect-[4/3] bg-gray-100">
                ...
                {item.title && (
                  <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/60 to-transparent p-3">
                    <p className="text-white text-xs font-display font-medium line-clamp-1">{item.title}</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="mt-6 sm:hidden">
          <Link
            href="/gallery"
            className="flex items-center justify-center gap-2 w-full rounded-xl border border-[#F5B400] text-[#F5B400] py-3 font-display font-medium text-sm hover:bg-[#F5B400] hover:text-white transition-all duration-200"
          >
            View full gallery
            <ArrowRight className="h-4 w-4" />
          </Link>
  ...
  return (
    <div className={`relative overflow-hidden rounded-2xl bg-gray-100 border border-gray-100 shadow-sm group select-none ${className}`} onContextMenu={(e) => e.preventDefault()}>
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-400 flex items-end p-4">
        {item.title && (
          <p className="text-white font-display font-medium text-sm">{item.title}</p>
        )}
      </div>
    </div>
  );
